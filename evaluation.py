...
from inference import build_user_prompt, SYSTEM_PROMPT, TEMPERATURE, MAX_TOKENS, get_agent_action
...
def run_episode_sync(env_url: str, task_id: str, client: OpenAI) -> float:
    ...
    reset_resp = requests.post(f"{env_url}/reset", json={"task_id": task_id}, timeout=15)
...
async def run_episode(task_id: str) -> type("Result", (), {"reward": float}):
...
async def evaluate_agent(model_name: str, n_episodes: int = 5):
    ...
    results = {}
    print(f"Starting evaluation for model '{model_name}' on {len(SCENARIOS)} tasks, {n_episodes} episodes each.\n")
    ...
    for task_def in SCENARIOS:
        ...
        success_rate = sum(1 for s in scores if s >= SUCCESS_SCORE_THRESHOLD) / len(scores)
        ...
        print(f"  -> Task Summary: Success Rate {success_rate*100:.1f}%, Avg Score {avg_score:.2f}\n")
